...
export default async function FloatingContactTabs() {
  const supabase = await createClient();
  const { data } = await supabase
    .from('site_contacts')
    .select('updates_giveaway_url, whatsapp_channel_url, customer_support_url')
    .eq('id', 1)
    .maybeSingle();

  const items = [
    { label: 'UPDATE & GIVEAWAY', href: data?.updates_giveaway_url || '', className: 'telegram', icon: <TelegramIcon /> },
    { label: 'WHATSAPP CHANNEL', href: data?.whatsapp_channel_url || '', className: 'whatsapp', icon: <WhatsAppIcon /> },
    { label: 'CUSTOMER SUPPORT', href: data?.customer_support_url || '', className: 'support', icon: <SupportIcon /> },
  ];
