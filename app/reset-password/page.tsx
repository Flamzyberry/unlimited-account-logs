...
export default function ResetPasswordPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    setTimeout(() => {
      router.replace(profile?.role === 'admin' ? '/admin/login' : '/login');
    }, 1000);
  }
    ...
    <main className="container" style={{ padding: '60px 0' }}>
      <div className="card" style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        <form onSubmit={submit} style={{ display: 'grid', gap: 14, marginTop: 24 }}>
          <input required minLength={8} value={password} onChange={e => setPassword(e.target.value)} placeholder="New password" type="password" style={{ padding: 13, border: '1px solid #d1d5db', borderRadius: 10 }} />
          <input required minLength={8} value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Confirm new password" type="password" style={{ padding: 13, border: '1px solid #d1d5db', borderRadius: 10 }} />
