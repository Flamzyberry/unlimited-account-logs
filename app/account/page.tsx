...
export default async function AccountPage() {
  ...
  const { data: profile, error: profileError } = await supabase
    .from('profiles').select('email, full_name, phone, role, created_at')
  ...
  return (
    <main className="container" style={{ padding: '40px 0' }}>
      <div className="nav">
        ...
        <form action="/auth/signout?next=/login" method="post"><button className="btn secondary">Sign out</button></form>
      </div>
      <section className="section">
        <div className="card" style={{ maxWidth: 600 }}>
          <h1>Welcome{profile.full_name ? ', ' + profile.full_name : ''}</h1>
          <p style={{ color: '#6b7280' }}>Your customer authentication is active.</p>
